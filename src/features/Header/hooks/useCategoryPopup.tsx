...
export function useCategoryPopup(){
  ...
  const [, dispatchCategory] = useCategoryContext();
  const [mainCategoryOptions, setMainCategoryOptions] = useState<MainCategory[]>([]);
  const [subCategoryOptions, setSubCategoryOptions] = useState<SubCategory[]>([]);
  // 방향키 조작시, 선택된 카테고리를 의미하는 상태 (비활성화됨)
  const [active] = useState<CategoryPopupType>({categorySection : "none", idx : 0});
  ...
  //   countsRef.current = {
  //     ...countsRef.current,
  //     sub: subCategoryOptions.length ?? 0,
  //   };
  // }, [subCategoryOptions]);
  ...
  //           dispatchCategory({ type: "SET_MAINCATEGORY", payload: mainCategoryOptions[activeRef.current.idx]});
  //         }
  ...
  //           dispatchCategory({ type: "SET_MAINCATEGORY", payload: mainCategoryOptions[cur.idx]});
  //           setActive({ categorySection: "sub", idx: 0 });
  //         }
  //         // 서브 선택
  ...
  //               payload: subCategoryOptions[cur.idx]
  //             });
  //           setOpenPopup(false);
  //         }
  //         break;
  ...
  const onMainSelect = (text : MainCategory, _idx: number) => {
    dispatchCategory({ type: "SET_MAINCATEGORY", payload: text});
    // setActive 비활성화
    // setActive({ categorySection: "sub", idx: 0 });
  }

  const onSubSelect = (text : SubCategory, _idx: number) => {
    dispatchCategory({ type: "SET_SUBCATEGORY", payload: text});
    setOpenPopup(false);
  }

  // 마우스 호버 시 active 상태 업데이트 (비활성화됨)
  const onMainHover = (_idx: number) => {
    // setActive({ categorySection: "main", idx });
  }

  const onSubHover = (_idx: number) => {
    // setActive({ categorySection: "sub", idx });
  }

  return {panelRef, toggleRef, active, onMainSelect, onSubSelect, onMainHover, onSubHover,
    mainCategoryOptions, setMainCategoryOptions, subCategoryOptions, setSubCategoryOptions};
}