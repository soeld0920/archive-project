...
export default function UserInfo({userUUID} : UserInfoProps){
  //"" = 로그인 X
  if(userUUID === "") return (
  ...
  if(!user) return <Navigate to="/404" replace/>;
    const {image,nickname,totalWriting,totalComment,totalView,totalGreat} = user;
  return (
  <div className={styles.loginWrapper}>
    <div className={styles.profilePicture}>
      <img src={image} alt={`${nickname}의 프로필 이미지`} loading="lazy" onError={(e) => (e.currentTarget.src = 'src/assets/img/profile-fallback.png')}/>
    </div>
    <div className={styles.profileInfo}>
